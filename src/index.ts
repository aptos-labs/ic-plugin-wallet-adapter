...
import type { ICDappClientConfig } from '@identity-connect/dapp-sdk/dist/client';
...
export class IdentityConnectWallet implements AdapterPlugin {
  ...
  readonly provider: ICDappClient;

  constructor(dappId: string, options?: ICDappClientConfig) {
    this.provider = new ICDappClient(dappId, options);
  }

  private async getConnectedAccount() {
    const accounts = await this.provider.getConnectedAccounts();
  ...
  async connect(): Promise<AccountInfo> {
    ...
    for (const account of accounts) {
      if (account.accountAddress === address) {
        ...
        await this.provider.disconnect(account.accountAddress);
      }
    }
  ...
  async disconnect(): Promise<void> {
    const account = await this.getConnectedAccount();
    if (account) {
      await this.provider.disconnect(account.address);
    }
  }
  ...
  async signMessage(message: SignMessagePayload): Promise<SignMessageResponse> {
    ...
    const { prefix, ...rest } = await this.provider.signMessage(account.address, message);
    if (prefix !== 'APTOS') {
