...
import {
  type JsonTransactionPayload,
  isBcsSerializableTransactionPayload,
} from '@identity-connect/wallet-standard';
...
const supportedJsonTypes = ['entry_function_payload', 'multisig_payload', undefined];
function isJsonTransactionPayload(payload: Types.TransactionPayload | TxnBuilderTypes.TransactionPayload): payload is JsonTransactionPayload {
  return 'type' in payload && supportedJsonTypes.includes(payload.type);
}
...
export class IdentityConnectWallet implements AdapterPlugin {
  ...
  async signAndSubmitTransaction(
    ...
    if (!isJsonTransactionPayload(payload)) {
  ...
  async signAndSubmitBCSTransaction(
    ...
    if (!isBcsSerializableTransactionPayload(payload)) {
      throw `${IcWalletName} Payload type not supported`;
    }
  ...
  async signTransaction(
    payload: Types.TransactionPayload | TxnBuilderTypes.TransactionPayload,
  ) {
    const account = await this.getConnectedAccount();
    if (!account) {
      throw `${IcWalletName} Account not paired`;
    ...
    if (!isJsonTransactionPayload(payload) && !isBcsSerializableTransactionPayload(payload)) {
      throw `${IcWalletName} Payload type not supported`;
    }
